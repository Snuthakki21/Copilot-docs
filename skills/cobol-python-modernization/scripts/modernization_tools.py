...
_REPORT_SECTIONS = ("Executive Validation Summary", "Rule Parity Matrix", "Detailed Discrepancies", "Final Verdict")
_REPORT_FIELDS = ("Source code evidence", "Source behavior", "Python code evidence", "Python behavior", "Validation evidence", "Parity result")
...
def _validation_report_errors(path: Path, rule_ids: list[str]) -> list[str]:
    if not path.is_file():
        return []
    try:
    ...
    errors.extend(f"validation report missing field: {field}" for field in _REPORT_FIELDS if field not in text)
    for rule_id in rule_ids:
        if rule_id and rule_id not in text:
            errors.append(f"validation report omits Rule ID: {rule_id}")
    return errors
...
def validate_job_artifacts(job_dir: Path) -> dict[str, object]:
    ...
    if not isinstance(rules, list) or not rules:
        ...
        for idx, rule in enumerate(rules, start=1):
            if not isinstance(rule, dict):
                ...
                continue
            rule_id = str(rule.get("id") or f"#{idx}")
    ...
    errors.extend(_validation_report_errors(job_dir / "validation_report.md", rule_ids))
    ...
    for key, label in (("security_findings", "security findings"), ("defects", "defects"), ("unresolved", "unresolved source behavior"), ("unsupported_claims", "unsupported validation/review claims")):
