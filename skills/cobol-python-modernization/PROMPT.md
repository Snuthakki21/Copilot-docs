...
For validation, produce an evidence-only side-by-side rule comparison. For every Rule ID show source code evidence, source behavior in plain English, Python code evidence, Python behavior in plain English, validation/test evidence, and PASS/FAIL/UNVERIFIED. Never invent a root cause or remediation. If evidence is insufficient, state `UNKNOWN — UNVERIFIED`, record the gap in `traceability.json.unsupported_claims` or `unresolved`, and block 100%.

Run functional validation with source-derived fixtures and SQLite, obtain at least 80% coverage across generated `.py` files, then perform the independent adversarial review plus final security/defect checks. A job may receive **100%** only when every discovered behavior is accounted for and mirrored, every rule is traced and tested, documentation/provenance gates pass, coverage is at least 80%, reconciliation passes, and there are zero unresolved migration mismatches, unsupported claims, or introduced security/functional defects. Otherwise fail closed and score 0%.
